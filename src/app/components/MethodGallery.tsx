...
import { methodGalleryItems, methodGalleryCopy } from "../i18n/content";

// El orden de imágenes/spans/beforeAfter debe coincidir con methodGalleryItems en content.ts
const images = [botox, labios, rino, ojera, mandibular, surcos, pomulos, capilar, manos, microderm, esclero];
const meta: { beforeAfter?: boolean; span?: string }[] = [
  { beforeAfter: true, span: "lg:row-span-2" },
  { beforeAfter: true },
  { beforeAfter: true },
  { beforeAfter: true, span: "lg:row-span-2" },
  { beforeAfter: true },
...
export default function MethodGallery() {
  ...
  const copy = methodGalleryCopy[lang];
  const items = methodGalleryItems[lang].map((it, i) => ({ ...it, img: images[i], ...meta[i] }));
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-8">
        ...
        >
          <div className="flex items-center gap-3 mb-5">
            ...
            <span className="text-[12px] tracking-[0.18em] uppercase text-cyan font-semibold">{copy.eyebrow}</span>
          </div>
          <h2 className="font-display font-normal text-[30px] md:text-[40px] leading-[1.16] tracking-tight text-ink">
            {copy.heading}
          </h2>
          <p className="text-[15.5px] text-ink-soft leading-relaxed font-light mt-5">{copy.description}</p>
        </motion.div>
          ...
          {items.map((item, i) => (
            ...
            >
              ...
              <div className="absolute bottom-0 left-0 right-0 p-4">
                <h3 className="text-white font-display text-[16px] leading-tight mb-0.5">{item.title}</h3>
