...
import { videoSectionCopy } from "../i18n/content";

export default function VideoSection() {
  const { lang } = useLanguage();
  const c = videoSectionCopy[lang];
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-8">
        ...
        >
          <div className="flex items-center gap-3 mb-5">
            ...
            <span className="text-[12px] tracking-[0.18em] uppercase text-cyan font-semibold">{c.eyebrow}</span>
          </div>
          <h2 className="font-display font-normal text-[30px] md:text-[40px] leading-[1.16] tracking-tight">
            {c.heading}
          </h2>
          <p className="text-[15.5px] text-white/60 leading-relaxed font-light mt-5">{c.description}</p>
        </motion.div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 border border-white/10 divide-y divide-white/10 lg:divide-y-0 lg:divide-x">
          {c.steps.map((step, i) => (
            ...
            >
              ...
              <h4 className="font-display text-[19px] font-medium mb-3">{step.title}</h4>
              <p className="text-[13.5px] leading-relaxed text-white/60 font-light">{step.desc}</p>
            </motion.div>
          ))}
        </div>
      </div>
