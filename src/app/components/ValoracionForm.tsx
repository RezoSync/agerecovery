...
interface ValoracionFormProps {
  /** Título mostrado sobre el formulario. Si se omite, usa el título por defecto traducido. */
  title?: string;
  ...
  /** Lista de opciones para el select cuando lockInterest es false. Si se omite, usa las opciones traducidas por defecto. */
...
export default function ValoracionForm({
  ...
  const finalTreatmentOptions = treatmentOptions ?? [...dict.form.defaultOptions];
  const finalTitle = title ?? t("form.defaultTitle");

  const [name, setName] = useState("");
  const [phone, setPhone] = useState("");
  const [interest, setInterest] = useState(presetInterest ?? finalTreatmentOptions[0]);
  ...
  function handleSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    const phoneError = validatePhone(phone, lang);
    const interestError = validateInterest(finalInterest, lang);
  ...
  return (
    <GlowCard className={`p-8 ${className}`}>
      <h3 className="text-2xl mb-6 text-white font-display">{finalTitle}</h3>

      {sent ? (
        <div className="flex flex-col items-center gap-4 py-12 text-center">
          <CheckCircle2 size={48} className="text-cyan" />
          <div>
            <div className="text-lg font-medium mb-1 text-white font-display">{t("form.sentTitle")}</div>
            <div className="text-sm text-white/60">{t("form.sentDescription")}</div>
          </div>
          <button
            onClick={() => {
              setSent(false);
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-5">
          ...
          <p className="text-xs text-white/40 leading-relaxed">{t("form.disclaimer")}</p>
        </form>
      )}
