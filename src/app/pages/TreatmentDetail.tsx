...
export default function TreatmentDetail() {
  ...
  useEffect(() => {
    ...
    // Ambas reciben el idioma activo para devolver el contenido correcto.
    async function loadPageData() {
      ...
      try {
        const data = await loadTreatmentWithRetry(currentSlug, 2, lang);
        if (!isMounted) return;
        setTreatment(data);

        const relatedData = await fetchRelatedTreatments(currentSlug, 3, lang);
        if (!isMounted) return;
        setRelated(relatedData);
      } catch {
        if (isMounted) {
          setError(t("treatmentDetail.loadError"));
        }
      } finally {
        if (isMounted) {
          setIsLoading(false);
  ...
  useSEO({
    title: treatment ? treatment.title : t("treatmentsList.seoTitle"),
    description: treatment ? treatment.shortDesc : t("treatmentDetail.seoFallbackDescription"),
  ...
  return (
    <Layout
      breadcrumbs={[
        { label: t("nav.home"), to: "/" },
        { label: t("nav.treatments"), to: "/tratamientos" },
        { label: treatment ? treatment.title : t("treatmentDetail.loadingShort") },
      ]}
    >
      <section className="pt-28 md:pt-36 pb-12 md:pb-16">
        <div className="max-w-7xl mx-auto px-6 md:px-8">
          {isLoading && (
            <div className="py-24 text-center text-white/60 text-[14px]">{t("treatmentDetail.loading")}</div>
          )}
          ...
          {error && (
            <div className="py-24 text-center">
              ...
              >
                {t("treatmentDetail.backToCatalog")}
              </Link>
            </div>
          )}
            ...
            <>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-px bg-white/10 border border-white/10 mb-16">
                <div className="bg-navy-card p-7">
                  ...
                  <div className="font-display text-[16px] text-white">{treatment.duration}</div>
                </div>
                <div className="bg-navy-card p-7">
                  <div className="text-[11px] tracking-wide uppercase text-cyan font-semibold mb-2">
                    {t("treatment.sessions")}
                  </div>
                  <div className="font-display text-[16px] text-white">{treatment.sessions}</div>
                </div>
                <div className="bg-navy-card p-7">
                  <div className="text-[11px] tracking-wide uppercase text-cyan font-semibold mb-2">
                    {t("treatment.results")}
                  </div>
                  <div className="font-display text-[16px] text-white">{treatment.results}</div>
                </div>
              </div>
                ...
                <div className="lg:col-span-2">
                  <TreatmentTabs
                    tabs={[
                      {
                        id: "beneficios",
                        label: t("treatmentDetail.tabBenefits"),
                        content: (
                          <section>
                      ...
                      {
                        id: "procedimiento",
                        label: t("treatmentDetail.tabProcedure"),
                        content: (
                          <section>
                      ...
                      {
                        id: "recuperacion",
                        label: t("treatmentDetail.tabRecovery"),
                        content: (
                          <section>
                      ...
                      {
                        id: "para-quien",
                        label: t("treatmentDetail.tabIdealFor"),
                        content: (
                          <section>
                            ...
                            <div className="border border-white/10 bg-navy-card p-6">
                              ...
                              <p className="text-[12px] text-white/40 font-light mt-4">
                                {t("treatment.contraindicationsNote")}
                              </p>
                            </div>
                          </section>
                        ),
